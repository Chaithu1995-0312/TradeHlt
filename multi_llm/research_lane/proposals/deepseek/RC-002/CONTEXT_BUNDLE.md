...
Generated: 2026-09-03T20:45:16Z
...
definition quoted in §3(b) — **there is genuinely no character change.**

**Consequence:** on this evidence these 32 bars are **not** sweep-then-reversal events. The
founding sweep pointed *against* an already-prevailing trend and the qualifying candle moved
*with* that trend. That is trend **resumption** after a counter-trend sweep. `SweepReversal`
is a **misnomer** (name retained in `market_shapes.yaml` for traceability, corrected in
place, never silently deleted).

---

## 5. The base-rate control — run for this cycle, and it cuts both ways

The retraction above originally shipped with **no denominator**. Both of its statistics have
now been measured against their corpus-wide base rate (all 2,038 bars, same artifact):

| Phase 4g statistic | Observed | Corpus base rate | Binomial P(>=obs given base) | Verdict |
|---|---|---|---|---|
| `trend_bias(t-1)` already aligned with candle dir(t) | **29 / 32 (90.6%)** | **47.86%** (974/2,035) | **4.05e-07** | **Strong. Survives.** |
| `trend_bias` did **not** flip at `t` | **30 / 32 (93.8%)** | **95.48%** (1,945/2,037) | **0.826** | **Vacuous. Withdrawn.** |

Read this carefully, because it is the single most decision-relevant block in the dossier:

- The **"didn't flip" statistic carries no information at all.** `trend_bias` almost never
  flips on any bar (4.5% of the corpus). Citing 30/32 as evidence was a second error, of the
  opposite kind to the first, and it is withdrawn here.
- The **"already aligned at `t-1`" statistic is real and large** — 90.6% against a 47.86%
  coin-flip base rate, p ~ 4e-07. The corrected *conclusion* in §4 therefore **survives its
  own control and is strengthened**, even though one of its two supporting numbers does not.

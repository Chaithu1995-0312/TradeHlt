...
You are the **fourth and last model** actor in cycle `{{CYCLE_ID}}`. Three packages precede
...
Return the **complete filled** `DECISION` markdown for `{{PROPOSAL_PATH}}`, using the

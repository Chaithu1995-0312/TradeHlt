...
try:
    ...
    from crt_engine_v2 import CRTConfig          # kept only for type hints
    from config_builder import ConfigBuilder      # REQUIRED — replaces get_crt_config
    from llama_gate import llm_score
...
CONSISTENCY_ALPHA  = 0.1   # penalty weight for cross-instrument score variance
...
def _compute_base_score(metrics: dict) -> float:
    ...
    if exp > 20 and ret > 0 and (ret / exp) < 0.05:
        return -999.0
    ...
    score = (0.50 * exp_rr) + (0.20 * wr) + (0.20 * tc_norm) + (0.10 * (1.0 - dd))
    return score
...
def fitness_single(metrics: dict, use_llm: bool = True) -> float:
    ...
    if base_score <= 0.1 or not use_llm:
    ...
    if llm_weight < 0.2:
        tuner_log.info(f"LLM hard-rejected config (base={base_score:.3f}).")
        return -999.0

    return round(base_score * (0.7 + 0.3 * llm_weight), 6)
...
class AutoTuner:
    ...
    def run(self) -> TunerResult:
        ...
        print(f"\n{'═'*65}")
        print(f"  CRT AUTO-TUNER [v3 — {mode_label}]")
        print(f"  Instruments:  {', '.join(self.csv_paths)}")
        for inst, total in self.total_candles_map.items():
            end = self.train_end_indices[inst]
            if self.train_split < 1.0:
                print(f"  {inst:<12} candles={total:,}  IS=0→{end}  OOS={end}→{total}")
            else:
                print(f"  {inst:<12} candles={total:,}")
    ...
    def _save_final_report(self) -> None:
        ...
        if best:
            ...
            for inst, data in best.instrument_results.items():
                print(f"    {inst:<12} score={data.get('score', -999):>+.4f}")
            print(f"{'═'*65}\n")

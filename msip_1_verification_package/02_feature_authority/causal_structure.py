...
No body/range/wick geometry. Liquidity distance routes through derived_math.
"""
...
def causal_structure_series(
    ...
    *,
    ...
    for i in range(n):
        lo = max(0, i - _DOUBLE_SWEEP_WINDOW + 1)
        window = sweep[lo : i + 1]
    ...
    for i in range(n):
        ...
        d = dm_liquidity_distance(float(close[i]), float(atr[i]), *levels)
        if d != d:
            d = 10.0
        liq_dist[i] = d
        liq_press[i] = dm_liquidity_pressure_score(d)
...
def causal_structure_at_bar(
    ...
    *,
    ...
    if liquidity_sweep_history is not None:
        ...
        hist = hist[-_DOUBLE_SWEEP_WINDOW:]

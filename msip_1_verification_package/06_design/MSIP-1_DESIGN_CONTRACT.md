...
**Status:** PROPOSED UNDER VERIFICATION — **CORRECTED 2026-07-14** (not implemented; not authorized)  
**Package generated:** 2026-07-14T07:08:43Z  
**Correction:** 2026-07-14 (post XAUUSD CRT baseline + transition traces)  
...
> **CORRECTION (PROVEN_BY_EXECUTABLE_EVIDENCE):** Current CRT transition logic is **not** downstream of the 38-dim FeaturePipeline vector. Full write-up:  
> `docs/governance/MSIP-1_DESIGN_CONTRACT_CORRECTION-2026-07-14.md`  
> Matrices: `docs/governance/CRT_INPUT_AUTHORITY_MATRIX_V1.json`,  
> `docs/governance/CRT_TRANSITION_COVERAGE_MATRIX_V1.json`.
...
### 2.0 CURRENT executable architecture (corrected)

```text
OHLCV ──► FeaturePipeline ──► 38 vector ──► journal @ TRADE_OPENED / models
OHLCV ──► RAW OHLC + CRT-local math + state memory + CRTConfig
              ──► CRT process_candle ──► guards / transitions / trades
```

MSIP must **not** assume CRT already reads the 38-vector.

### 2.1 Proposed MSIP layering (shadow-first)

```text
OHLCV
  → FeaturePipeline / candle_math / derived_math   (FEATURE AUTHORITY — existing)
  → MarketStateInterpreter (NEW, proposed)         (MSIP — compose; SHADOW ONLY first)
        → MarketStateVector
        → bar-by-bar comparison vs CURRENT CRT path
        → (FUTURE only, Authority Ladder) optional CRT consumer migration

Parallel (unchanged until explicit cutover):
  OHLCV → CRT-local path → CRT state machine → live decisions
```

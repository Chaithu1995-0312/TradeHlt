...
| F-048 | ARCH | **[CANDIDATE — intent pending]** The DecisionEngine RR gate consumes a candle-polarity score against a reward/risk threshold: RREngine `rr_ratio` = polarity ∈[0.5,1] (legacy field name, "Candle Polarity Index formerly misnamed RR Engine", `semantic:candle_structure_quality`), but `engine_runner:957` wires it into DecisionEngine's `fusion["rr"] < rr_threshold=1.5` gate → `low_rr` fires every candle → `run()` can NEVER `execute` (0/70,002). Consumer intent = TRUE RR (`test_crt_fixes:224` injects `rr:2.0`); real RR check is in UltronRiskGate (SL/TP-derived, separate). `ExecutionPlanner:208` hard-gates on `run()=="execute"` → live path (`live_engine_hook`) can structurally never admit a trade (consistent w/ F-010). NOT a research/backtest defect (gate-OFF never calls run(), F-037; gate-ON uses run() as veto only). Found while validating the retracted F-047 7c harness — it's WHY run() can't measure body_ratio flips; body_ratio remediation BLOCKED behind it. Mismatch Certain; unintended-mis-wire vs dormant-live-by-design = open. Authority: architecture/governance only | Likely |
...
| RR lineage | **AUDITED** | Three contracts (polarity / rr_fusion inert / F-048 mismatch); not remediated | [`docs/governance/rr_lineage_audit.md`](docs/governance/rr_lineage_audit.md) | no | AUDITED ≠ CLOSED |
...
| TradeNet lineage | **AUDITED** | INERT / UNWIRED fusion neural slot (F-005) | [`docs/governance/tradenet_lineage_audit.md`](docs/governance/tradenet_lineage_audit.md) | no | AUDITED ≠ CLOSED |

...
| F-072 | ARCH | Canonical `atr` is close-relative (FM-041, `atr_14_raw/close`) but `compute_crt_levels` (`gate_intelligence.py:24-84`) requires price-unit ATR for SL/TP geometry. 3 sites feed the ratio into price-unit arithmetic: `live_engine_hook.py:916`, `research/model_runners/adapters/execution_plan.py:279-287`, `config_layer/execution_planner.py:390,403,406`. On XAUUSD the SL buffer is off by ~2000× (inflating `position_size_hint` identically); masked on EURUSD (~8% error). Backtest path correct (`crt_engine_v2.py:2201` uses `state.atr_abs`). OPEN — currently latent (no live caller, see F-073); remediation is a behavior-change decision, not mechanical. Authority: architecture/governance only | Certain |

...
| F-057 | ARCH | CRTConfig resolution is split-brain on the programmatic path: `BacktestRunner(cfg)` with `crt_config is None` (`backtest_v2.py:1633`, tuner/tests/embedders) calls `ConfigBuilder.build()` with no overrides → `market_router` hardcoded FOREX/CRYPTO profiles win, production JSON never opened. `expansion_atr_min_distance` 0.08 vs JSON 0.30 (3.75×) on the DISPLACEMENT→EXPANSION gate. CLI path is fine. OPEN — fix = backtest Phase 2. Authority: architecture/governance only | Certain |

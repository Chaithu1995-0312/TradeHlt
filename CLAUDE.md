...
| F-064 | ARCH | F-061's FM-022/023 dimensional-mix decision-surface defect GENERALIZES to XAUUSD (4th instrument, non-crypto): `|tanh(momentum_score)|>0.999` on 99.70% of 19,922 bars (re-measured 2026-09-16 on the full 47,197-bar corpus: 99.81%), `|ema_spread|>0.15` on 99.99%, exact 100.000× scale ratio under a synthetic 100× price shift (vs 1.000× for every other ATR-normalized dim) — confirms `legacy == corrected * close` off crypto. Measured, not activated: `normalization_basis` stays `atr_relative` by explicit decision; a one-time startup log warning added (log-only, zero emitted-value change). DESCRIPTIVE only, grants NO activation authority (§6.5) | Certain |
...
| F-101 | GOV | One backtest run mints THREE independently-computed run identifiers via two distinct compounding mechanisms (real ~5min time gap at RUN_ID vs config-dump/ReportWriter mint time; UTC vs naive-local clock-basis mismatch between the latter two) — not joinable by string equality; only ReportWriter's id reaches the v3/v4 sidecars | Certain |
| F-102 | RISK | EngineRunner's post-commit gate (TRADE_OPENED-only) fails OPEN on any exception (DEBUG-only log, `_engine_vetoed` never set) — confirmed by forced-injection proof (3/3 injected exceptions let trades survive, 0 `engine_runner:` rejects in the real events stream); dormant (0/3 natural) on XAUUSD, one instance of the F-105 silent-except class | Certain |
| F-103 | ARCH | Backtest and live run on two rails with disjoint decision-plane reachability, confirmed by AST import-graph assertion: backtest reaches CRT/ParentCRT/HTFState but never ExecutionPlannerV1_2/UltronRiskGate; live reaches the latter but has no CRT state machine anywhere in its call graph — sharpens F-073 | Certain |
| F-104 | ARCH | FeaturePipeline drops only its declared leading warmup on XAUUSD M15 (78/78, 0 mid-file) — the fail-closed alignment guard the code names as a mid-file-drop risk is unexercised on this corpus; that guard only fires on TRADE_OPENED bars (F-102), so the structural gap stays latent, not closed | Likely |
| F-105 | GOV | 9 of 13 broad `except Exception` blocks in the backtest per-bar loop are silent swallows (AST census, `BacktestRunner.run`) — 3 carry a documented `# fail-open` comment (StrategyOrchestrator consensus), 6 are undocumented and untriaged; F-102 is this class's EngineRunner-gate instance | Certain |
| F-107 | ARCH | Two canonical names contradicted their values — index 11 `trend_strength` was always a rolling z-score, index 35 `candles_since_retest` always counted bars since the last SWEEP (47,183/47,183). Renamed at schema v6.0 to `trend_strength_z` / `candles_since_sweep` (48 dims, same order); the four in-place z-scores moved to the raw/z split so `NORMALIZE_COLS` is now empty. Vector proved bit-identical BY POSITION over 47,197 bars — names moved, values did not. Executes the pin's deferred T-19. No authority, no retrain | Certain |
| F-108 | ARCH | The live-rail planner had no with-trend intent label — `REVERSAL` tested only the counter-trend half of EMA-vs-direction, so with-trend entries fell to `UNKNOWN` and were rejected on 40.70% (LONG) / 31.97% (SHORT) of XAUUSD M15 bars; three tests had encoded that hole as their "no pattern" fixture. Added `CONTINUATION`: UNKNOWN → 0.004% (exact EMA ties), no other label moved. Classifier-only by user decision — the gate scores `CONTINUATION` 0.0 (0.0% approval measured, not structurally guaranteed). Live rail only (backtest never imports the planner). No authority | Certain |
| F-106 | ECON | Conditional on M15 **engine** SWEEP occupancy (`process_candle`, not FeatureState `sweep_detected` — overlap 927/1792=51.7%), ATR 1:2 (`forward_walk` 40 bars) is indistinguishable from random entry (TP 32.9% vs 32.9%; exit P(MFE≥1R) 50.9% vs 51.0%; independence PASS). Session split is CRT filter hours, not FM-052. Median −1R on planner/two_r/atr/random. Random-parity scoped to ATR 1:2 only. Extends F-086. No G001 | Likely |

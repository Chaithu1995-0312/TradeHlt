...
# ── shared factory (research-framework consolidation Phase 3, 2026-09-14) ──────────────────────
def xau_measured_cost_model() -> ComponentCostModel:
    """The F-082 measured XAUUSD broker calibration — the `_XAU_COST` literal that was
    byte-identically duplicated in `research.mother_range.driver` and `research.sujan_crt.driver`.
    A single shared instance so the two never silently diverge."""
    return ComponentCostModel(
        ...
        status="MEASURED",
        entry_slippage_basis=ENTRY_SLIP_PROXY_FROM_STOP,
    )

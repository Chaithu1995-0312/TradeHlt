# -*- coding: utf-8 -*-
"""
qualify_fx_metals.py — "Run the Gate First" on the FX universe (thin CLI).

Sibling of qualify_majors.py, re-scoped from crypto majors to five FX majors
(EURUSD, AUDUSD, EURCAD, GBPUSD, USDJPY — 2yr M15 fetched via MT5). Reuses the M4
gate math in research.qualification VERBATIM (evaluate_pre_bh / benjamini_hochberg
/ finalize) and HypothesisRunner. Adds NO statistics; only re-scopes the existing
pooled QualificationGate over

    [EURUSD, AUDUSD, EURCAD, GBPUSD, USDJPY, POOLED]   (per-instrument first, pooled last)

and over the two config families (toy / spine) exactly as qualify_majors does.

  * toy family   — configs/research/research_config_fx_metals.json        (apply_signal_defaults=true)
        candidates: expansion_breakout, mean_reversion   vs falsification controls
  * spine family — configs/research/research_config_spine_fx_metals.json   (apply_signal_defaults=false)
        candidate:  spine (production composite, prod_version=v2_multi_2026_04) vs controls

XAUUSD is excluded from this pass (it REJECTs the FX-tuned dataset_integrity gap
gate pending the Stage-2 metals/holiday calendar). FX is the cross-asset-class test
of the F-019..F-028 entry-information null beyond crypto.

MEASURE-ONLY — no optimization, no new hypotheses, no promotion, no spine/config edits.
The deterministic JSON body carries NO wall-clock (matches qualify_majors); the run
manifest (timestamp / git) is written separately.

Usage:
    python scripts/research/qualify_fx_metals.py
    python scripts/research/qualify_fx_metals.py --out results/research/qualification_fx_metals
"""
...
SPINE_CONFIG = "configs/research/research_config_spine_fx_metals.json"
...
def _print_table(report_doc: dict) -> None:
    safe_print("\nQUALIFY-FX — per-instrument first, pooled last "
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="qualify_fx_metals",
        description="Cross-instrument M4 qualification of the existing hypothesis pool (FX majors)")
    parser.add_argument("--out", default="results/research/qualification_fx_metals")
    ...
    (out_dir / "qualify_fx_metals.json").write_text(
    ...
    (out_dir / "qualify_fx_metals_manifest.json").write_text(
        json.dumps(manifest, sort_keys=True, indent=2), encoding="utf-8")

    _print_table(report_doc)
    safe_print(f"\n-> {out_dir / 'qualify_fx_metals.json'}")
    return 0

...
from feature_builder import N_FEATURES, validate_vector, feature_dict_to_vector
...
MIN_RECORDS_TO_TRAIN  = 200   # absolute minimum for any training run
MIN_RECORDS_RECOMMEND = 500   # below this: warn, don't abort
...
def validate_logs(
    *log_paths: str | Path,
    ...
    for tid, entry in entries.items():
        ...
        ok, reason = validate_vector(vec)
        if not ok:
            rpt.skipped_bad_features += 1
            rpt.warnings.append(f"trade_id={tid[:8]}… bad features: {reason}")
            continue

...
def find_active_order_block(window: "Sequence[Candle]", k: int) -> Optional[Zone]:
...
def order_block_distance(window: "Sequence[Candle]", k: int, atr: float) -> float:
    ...
    zone = find_active_order_block(window, k)
    if zone is None:
        return 0.0

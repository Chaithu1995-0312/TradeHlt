...
def find_active_breaker(window: "Sequence[Candle]", k: int) -> Optional[Zone]:
...
def breaker_distance(window: "Sequence[Candle]", k: int, atr: float) -> float:
    ...
    zone = find_active_breaker(window, k)
    if zone is None:
        return 0.0

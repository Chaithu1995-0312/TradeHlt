...
- [`src/config_layer/crt_engine_v2.py:87-95`](../../src/config_layer/crt_engine_v2.py) — `Candle` dataclass — the OTHER OHLCV consumer (7 fields, no feature payload); `body_size`/`wick_size`/`body_ratio` are `@property` derivations through `candle_math` (F-046), not stored columns. Per-bar sibling to the batch `feature_vectors` row, joined only at `TRADE_OPENED` — see below.
- [`src/runtime/backtest_v2.py:2981-3038`](../../src/runtime/backtest_v2.py) — the timestamp-string join (`feature_ts_to_idx`, a `{"YYYY-MM-DD HH:MM:SS": row_index}` dict, not a positional/`candle_idx` join — the old integer-offset version silently misaligned by ~50 bars post-dropna). A lookup miss raises `FeatureAlignmentError` (T-16, imported at `:43`) instead of the pre-T-16 zero-fill fallback.
- [`src/runtime/backtest_v2.py:322-392`](../../src/runtime/backtest_v2.py) — `TradeRecord` — the schema behind `XAUUSD_trades.csv` (unrelated same-named class also exists at [`src/journal/schema.py:8`](../../src/journal/schema.py), not the one in play here). Its `features: dict` is the 48-dim batch vector re-zipped from `CANONICAL_FEATURES`; `live_atr`/`live_ema_*`/`cached_*` are a **separate, third** OHLC-derived channel sampled live from `CRTEngine.get_live_metrics()` at entry, not from `feature_vectors`.
- [`src/runtime/backtest_v2.py:1693-1727`](../../src/runtime/backtest_v2.py) — `ReportWriter.write_all` — fans out to `summary.json` / `trades.csv` (`TradeRecord` flattened by `csv.DictWriter`, header = `rows[0].keys()`) / `events.jsonl` (compact `event/timestamp/candle_index/state_from/state_to/direction/price/reason/metadata` schema — **no OHLC or feature payload at all**). The two are deliberately not 1:1-joinable — see `src/research/evidence/catalog.py`'s `LEGAL_JOINS`/`IllegalJoinError` (`:116-147`).
- [`src/runtime/backtest_v2.py:397`](../../src/runtime/backtest_v2.py) — `_canonical_run_id` (UTC-minted, uncommitted as of 2026-09-16) — a **fourth** run identifier, stamped onto `summary.json`/`trades.csv`/`events.jsonl` via `write_all(..., run_id=...)`; in-progress remediation of the F-101 join-failure this doc's Discussion entry below was measured against.
...
- **2026-09-16 — end-to-end schema walk, OHLC → output files (session narrative, no behavior change).** Traced the full column-shape journey on `run_20260915_224051_XAUUSD` (XAUUSD M15, 47,275→47,197 rows) at the user's request. Two forks off the same 6-column CSV: `CandleLoader.stream()` → `Candle` (7 fields, per-bar) vs `FeaturePipeline.run()` → `(N,48)` `feature_vectors` (batch, dropna-trimmed). They rejoin only at `TRADE_OPENED` via the timestamp-string `feature_ts_to_idx` dict (not `candle_idx` — see Code covered). The joined `(48,)` vector is independently re-zipped into a `{name: value}` dict at **three** separate call sites in the per-bar loop (Phase-5 scorer, EngineRunner gate `_feat_map_er`, `TradeRecord.features`) — same pattern, not shared code. `TradeRecord` itself carries a second, independent OHLC-derived channel (`live_atr`/`cached_*` from `CRTEngine.get_live_metrics()`) alongside the batch one. Output fan-out (`ReportWriter.write_all`) produces two schema-incompatible files — `trades.csv` (86 cols, full feature payload — CORRECTED 2026-09-16: was "90", an unchecked estimate; verified by counting the header of `run_20260915_224051_XAUUSD/XAUUSD_trades.csv`) and `events.jsonl` (9 cols, no OHLC) — which is exactly why `catalog.py` refuses to 1:1-join them. Incidentally reproduced F-101 live: this run's own `h1_run_id_census.json` found `trades.csv`/`events.jsonl`/`summary.json` all lacked a `run_id` field; the uncommitted working-tree diff (dated 2026-09-16 in its own docstring) fixes exactly that with `_canonical_run_id`. No code changed by this entry — pure documentation of the existing (and one in-flight) path.
- **2026-09-16 — full-corpus feature block persisted as Parquet + xlsx, queryable in DuckDB.** `scripts/research/build_bar_matrix.py` now also writes `features.parquet` (and, with `--xlsx`, `features.xlsx`) beside `bar_matrix.parquet`: the pipeline block only — every `FeaturePipeline.run()` column plus `_pos` = **91** columns (90 pipeline + `_pos`; the 48 canonical are a subset), 47,197 rows on XAUUSD M15, from the same pass. The manifest now records `feature_columns`, `git_sha`, `tree_dirty`, and a per-writer `*_written` flag. `scripts/analysis/query_trace.py` gained the `bar_matrix_features` family (refused if `features_parquet_written` is not true; lineage from the same `manifest.json`). Verified: an independent pipeline recompute equals `features.parquet` across all 91 cols × 47,197 rows; `features.parquet` == `bar_matrix.parquet[:91]`; a DuckDB `_pos` join matches 47,197/47,197; the xlsx reads back 47,198 rows × 91 cols with first/last rows equal; the rebuilt `bar_matrix.parquet` is Arrow-equal to the 2026-09-09 build (not stale). **Pre-existing defect surfaced, not fixed here:** `state__displacement_flag` / `state__retest_flag` / `state__rsi_state` are 100% null in both builds — the builder iterates all `stateful_features` but `FeatureStateEncoder.classify()` returns only vector-bound families (`feature_states.py:170-184`, docstring says non-vector ones need `classify_value`). The raw input columns in the feature block are populated; only those three encoded state columns are empty.
- **2026-09-16 — FeatureState vs engine SWEEP (observation, not a finding).** On the F-106 corpus (47,197 M15 bars; 1,792 engine SWEEP events): `liquidity_sweep` **refines** `sweep_detected` (NoSweep counts identical 39,655; SweepDetected = BuySide ∪ SellSide = 7,542 exactly, including on engine-SWEEP bars 490+437=927). Engine SWEEP is fully directional (`trend_bias` Neutral = 0; all-corpus Neutral = 3). Engine SWEEP ≠ FeatureState `SweepDetected` (927/1792 = 51.7%). F-106 session split is CRT filter hours, not FM-052. Census: [`docs/analysis/feature-state-census-2026-09-16.md`](../analysis/feature-state-census-2026-09-16.md). Encoder remains shadow-only (`src/features/feature_states.py`).
...
- **2026-09-16 — PRODUCTION FIX: two canonical slots renamed so the name matches the formula (schema v5.0 → v6.0, `CH-schema-v6-normalization-identity`).** Five emitted columns carried names that contradicted their values; the ontology already declared the true formulas, so the NAMES were the defect. Canonical vector index **11** `trend_strength` → **`trend_strength_z`** (the slot always carried a rolling z-score: `compute_normalization` overwrote the raw chain in place) and index **35** `candles_since_retest` → **`candles_since_sweep`** (it always grouped on `liquidity_sweep != 0`; measured equal to a recomputed bars-since-sweep on 47,183/47,183 bars and 0 on exactly the 7,542 sweep bars). Vector stays **48 dims in the same order**. The three non-canonical in-place z-scored columns moved to the same raw/z split: `bb_width`/`price_vs_ma20`/`price_vs_ma50` → `*_raw` + `*_z`, and the previously-destroyed raw trend chain is now emitted as `trend_strength_raw` (FM-084, **non-canonical by decision** — a price-unit non-stationary dim must not enter the model vector, the F-061/F-064 class). Enriched frame 90 → 94 columns. `NORMALIZE_COLS` is now **empty**: every normalized quantity writes to its own `_z` column via `NORMALIZE_TO_NEW_COL`, so in-place z-scoring (the v3.0 `macd_hist` defect) cannot recur. FM-064/FM-065 refined IN PLACE (same ids, `version` bumped); `candles_since_retest_state` (FM-070) and its BitNet call-boundary alias key are untouched — the two names are now disjoint at the source, which finally *executes* the freeze pin's own deferred item **T-19** ("candles_since_retest rename or ontology note"). **Parity proof (the load-bearing claim): the 48-dim vector compared BY POSITION over all 47,197 XAUUSD M15 bars is bit-identical**, both renamed columns carry identical values under the new name, and the freeze pin's `vector_sha256` regression still matches — names moved, values did not. `SCHEMA_HASH`/`FEATURE_ORDER_HASH` necessarily change (they hash the names): order hash `160c96c52b198a16` → `7901bb0d34f3d0af`. A read-side `SCHEMA_V5_ALIASES` map (never emitted, mirroring `SCHEMA_V3_ALIASES`) keeps pre-v6 records decodable. Model `feature_order`/`feature_schema` name lists migrated by `scripts/governance/remap_registries_v6.py` (ALIGNMENT_REMAP provenance, no weight/mu/sigma touched) — the models stay exactly as stale/inert as F-076 records. Incidental pre-existing defects **surfaced and recorded, not silently absorbed**: `_derive_trade_intent`'s `candles_since_*` read is dead (its `cached_features` dict never carried the key under EITHER name, so the `pullback` branch is unreachable — deliberately left dead, since binding it would change TP selection), and the fc05 dependency table still declared the v4.0-dead name `wick_size` (fixed to `candle_range`).

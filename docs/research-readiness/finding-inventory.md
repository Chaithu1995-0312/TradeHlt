...
| **A-7** | **[RESOLVED 2026-06-12 by R4 — additive RETEST_REPLAY emitter restored, behavior-neutral.]** **Telemetry method gap.** `TelemetryCollector` had no `on_retest_replay` — 3 `test_execution_planner_replay` failures. | **medium** | An execution-planner replay/explain path was broken; reduces decision auditability. | `test_execution_planner_replay.py:44` | low–medium | partial |

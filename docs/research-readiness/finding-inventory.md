...
| **A-5** | **Branch-mixed test suite.** TP3/v4 + frequency-boost tests (`tp3_enabled` kwarg, `_FREQ_BOOST`) run on `patch` and fail by design (F-016). ~16 failures pollute the green/red signal. | **medium** | A researcher running `pytest` sees red and cannot tell real regressions from branch mismatch — erodes the suite as a trust gate. | 5×`tp3_enabled`, 2×`_FREQ_BOOST`, `test_roi_gaps`, `test_promotion_engine_overrides` | medium (mark branch-specific / skip-on-patch) | partial |

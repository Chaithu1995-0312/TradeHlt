...
| **R2** | ✅ **DONE 2026-06-12.** Wired `bitnet_main_threshold` (crt_engine gates) + `feature_monitor.{hard,soft}_drift_z` (FeatureMonitor construction) to config. Behavior-neutral: BNBUSDT + SOLUSDT ledgers byte-identical pre/post; reachability flips exactly those 3 knobs HARDCODED_OVERRIDE→READ_AND_USED (now 0 overrides). `news_blackout_minutes` left inert (separate; the news gate is unwired). | med | done | low | A-2 / [config-reachability-report.md](config-reachability-report.md) |
| **R3** | ✅ **DONE 2026-06-12.** Capability-probe `skipif` on the branch-scoped tests (7 TP3 + 3 freq-boost in test_roi_gaps; 4 promotion-override in test_promotion_engine_overrides). 14 fenced → skip-with-documented-reason ("requires post-TP3 lineage"), 4 genuine tests still pass, 0 fail. Auto-reactivate on the lineage that has the feature. `pytest` is a clean gate on `patch` again. | med | done | low | A-5 |
| **R4** | ✅ **DONE 2026-06-12.** Restored `TelemetryCollector.on_retest_replay` as an additive, explicit-signature RETEST_REPLAY emitter (mirrors `on_decision_distance`; new `_retest_replay_records` list aggregated by `flush()`). Contract derived from the 3 test call sites (R4A) — off-spine (zero production callers). Behavior-neutral: BNBUSDT + SOLUSDT ledgers byte-identical; 3 replay tests red→green; determinism green. | med | done | low–med | A-7 |

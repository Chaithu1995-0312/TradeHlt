...
class _StubBacktestConfig:
    """Accepts any kwargs so new BacktestConfig fields don't break the stub."""
    def __init__(self, instrument="UNKNOWN", pip_size=0.0001, crt_config=None,
...
from training.auto_tuner_multi import (
    PARAM_SPACE,
    fitness_multi,
    fitness_single,
    params_to_key,
...
def test_determinism(tmp_path):
    ...
    with patch("training.auto_tuner_multi.CandleLoader", _StubCandleLoader), \
         patch("training.auto_tuner_multi.BacktestRunner", _StubBacktestRunner), \
         patch("training.auto_tuner_multi.BacktestConfig", _StubBacktestConfig):
...
def test_forex_vs_crypto_config(tmp_path):
    ...
    with patch("training.auto_tuner_multi.CandleLoader",   _StubCandleLoader), \
         patch("training.auto_tuner_multi.BacktestRunner", _StubBacktestRunner), \
         patch("training.auto_tuner_multi.BacktestConfig", _StubBacktestConfig), \
         patch("training.auto_tuner_multi.ConfigBuilder",  _StubConfigBuilder), \
         patch("training.auto_tuner_multi.CRTConfig",      _StubCRTConfig):
...
def test_zero_expansion_guard():
    ...
    from training.auto_tuner_multi import _compute_base_score
    metrics = {
...
@pytest.mark.xfail(
    reason="'get_crt_config(' appears in inline comment only — not a real call; text-scan too coarse",
    strict=False,
)
